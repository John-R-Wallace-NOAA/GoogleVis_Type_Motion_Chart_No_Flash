...
During the interactive demo, after (or before) hitting play, hover around with the mouse cursor in the plotting area then left-click on a country. To go back, left-click on the white background of the plotting area and then hover around with the cursor on top of where other currently invisible data occurs before hovering back to the white background. 
...
A strange new world, you direct the process, but sometimes you are just the monkey running code for AI and telling it what you see. If another user needs something customized, 
one approach is to just fork the repo, upload the necessary code file(s) and tell Claude or another AI what you need.

The examples above and others are in the [DSLabs_Gap_data_with_fertility.md](https://github.com/John-R-Wallace-NOAA/GoogleVis_Type_Motion_Chart_No_Flash/blob/main/More_Examples/DSLabs_Gap_data_with_fertility.md) file.
